...
function TagPage() {
    ...
    let stocksByTag = () => 
        stocks && stocks.filter((s) => s.tags.includes(tag))

    return (
        <>
        <h2>{tag}</h2>
        <Sort setSortedStocks={setSortedStocks} sortBy={sortBy} sortCheck={sortCheck} stocks={stocksByTag()} />
  ...
  {stocks ?  <TagStockTableBody stocks={sortedStocks ? sortedStocks : stocksByTag()}/> : <p>Loading..</p> }
